...
#define CHECKMAP(nbytes) do { if (indexptr + nbytes >= index_map.data + index_map.len) { ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "index underrun, offset 0x%0x", nbytes); goto GENERAL_ERROR;}  } while(0)
    ...
    case 3:
        if ( indexptr[0] == '/' ) { /* absolute path */
            ...
            size_t prefix_length = lastslash - (char*)path.data;
            if ( (mediafilename = (char *)ngx_pcalloc(r->pool, prefix_length + 1 + mediafilenamelen + 1)) == NULL ) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "Insufficient memory");
                goto GENERAL_ERROR;
            }
            memcpy(mediafilename, path.data, prefix_length);
            mediafilename[prefix_length] = '/';
            memcpy(mediafilename + prefix_length + 1, indexptr, mediafilenamelen);
        }
        break;
    default:
